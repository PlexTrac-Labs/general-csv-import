...
> **Run every command from the repository root** (the folder that holds `Pipfile`
> and `data/`), invoking the script as `src/main.py`. The importer code lives in
> `src/`; generated PTRACs, failed PTRACs, sample/perf data, and your input files
> live under `data/`. Runtime paths are anchored to the repo root (see
> `src/paths.py`), so output always lands in `data/`/`logs/` regardless of where
> you launch from.
...
```bash
pipenv run python src/main.py --type example_csv --data-file-path data/input_files/csv_data.csv --api-version 2.19.0
```

Process an entire folder of inputs in one run:
```bash
pipenv run python src/main.py --type example_csv --data-folder-path data/input_files --api-version 2.19.0
```

Optionally import generated PTRACs into PlexTrac:
```bash
pipenv run python src/main.py --headers-file-path header_mapping.csv --data-file-path input.csv --api-version 2.19.0 --import-to-plextrac --instance-url https://example.plextrac.com --username user@example.com --password password
```
...
```bash
pipenv run python src/main.py --headers-file-path header_mapping.csv --data-file-path data.csv --api-version 2.19.0
```
...
## Parser Layout
All importer code lives under `src/` (paths below are relative to it):
- `src/main.py`: command-line orchestration, input discovery, PTRAC generation, and optional PlexTrac import.
- `src/mappings.py`: parser type definitions, mapping dictionaries, data loaders, data validation, and temp-CSV builders.
- `src/csv_parser.py`: generic parser and PTRAC builder.
- `src/mapping_utils/dradis_utils.py`: shared Dradis CSV/ZIP/XML parsing and rich-text helpers.
- `src/utils/`: authentication, input, API request, data lookup, validation, and file helpers.
- `src/paths.py`: filesystem anchors (`PROJECT_ROOT`, `SRC_DIR`, `DATA_DIR`, `LOGS_DIR`, `CONFIG_FILE`).
- `src/config.yaml`: default configuration (all CLI flags can be set here by underscored name).
...
1. Create a `data/input_files/` folder in the repository root.
...
3. Point the agent at the work with a prompt like the following (or something
   ...
   the example data files in data/input_files/.
   ```
